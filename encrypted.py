...
Encrypted Services DDos Script!''') 
		...
		def starturl(): 
			...
			choice1 = input("\nAre you only hitting one faggot?[y/n] ")

			if choice1 == "n":
				ip_file = input("Type the file name that you created with the IP lined up:[ip.txt]")
				ips = open(ip_file).readlines()



			else:
				url = input("\n Who we smackin -> ").strip()
				...
				try:
					...
					print("dumbass nigga typed it wrong")
					starturl()
